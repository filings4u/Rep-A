...
function renderStepTwoLayoutMarkup() { 
 ...
 <p style="color: var(--slate, #64748b); font-size: 0.95rem; margin: 0;">Provide company identifier records, operational parameters, and target communications parameters.</p> </div> 
 ...
 <i class="fa-solid fa-spinner fa-spin" style="color: var(--primary, #10b981); margin-right: 8px;"></i> <span>Loading company structure questionnaire forms...</span> </div> 
 </div> 
 </div> 
 ...
 <button type="button" class="btn-wizard-main btn-wizard-nav-next notranslate" translate="no" onclick="if(typeof window.goToNextWizardStep === 'function') { window.goToNextWizardStep(3, event); }" style="background: #0a1f44; border: none; color: #ffffff; padding: 12px 32px; border-radius: 6px; font-size: 0.95rem; font-weight: 700; cursor: pointer; transition: all 0.2s ease; box-shadow: 0 4px 10px rgba(10, 31, 68, 0.15); display: inline-flex; align-items: center; justify-content: center;" > <span>Continue to Add Ons</span> <i class="fa-solid fa-arrow-right" style="margin-left: 6px;"></i> 
    ...
    if (typeof hydratorFunction === "function") { 
        ...
        try { 
            ...
            // Pass the specific element target directly down into the handler
...
} // Cleanly closed the primary renderStepTwoLayoutMarkup function scope
...
function saveWizardFormStatesVanilla() { 
    ...
    allInputElements.forEach(inputNode => { 
        ...
        // FIX: Explicitly bypass file input elements to prevent security rule script crashes
        ...
        if (uniqueDataKey) { 
            ...
            // Backup separate entry keys for recovery scripts and Supabase validation 
...
function runStepTwoLayoutInitialization() { 
    ...
    // FIX: Executing straight code only without dynamic fallback network overrides or structural guesswork
    if (typeof window.renderStepTwoLayoutMarkup === "function") { 
        window.renderStepTwoLayoutMarkup(); 
    } else { 
        console.error("[Step 2 Fatal] Core renderStepTwoLayoutMarkup function is missing from execution workspace.");
...
window.processStepTwoFunnelAdvancementGate = function(event) { 
    ...
    // FIX: Dynamically track and evaluate all active multi-part validation objects registered for this service step
    let aggregatedValidationResults = { isValid: true, errors: [] };
    const verificationPartKeys = [
        `${serviceSlugKey}-part1-validation`,
        `${serviceSlugKey}-part2-validation`,
        `${serviceSlugKey}-part3-validation`,
        `annual-reports-part1-validation`,
        `annual-reports-part2-validation`,
        `annual-reports-part3-validation`
    ];

    verificationPartKeys.forEach(keyName => {
        const activeValidatorObject = window.formRegistry ? window.formRegistry[keyName] : null; 
    ...
    if (!aggregatedValidationResults.isValid) { 
        ...
        // FIX: Map the text message strings directly back to the accurate input DOM node IDs from your sub-files
        const errorsToProcess = aggregatedValidationResults.errors.map(errText => { 
            let inferredFieldId = ""; 
            const lowerText = errText.toLowerCase(); 
            
            if (lowerText.includes("business name")) inferredFieldId = "ar_business_name"; 
            else if (lowerText.includes("business id")) inferredFieldId = "ar_business_id"; 
            else if (lowerText.includes("business type")) inferredFieldId = "ar_business_type"; 
            else if (lowerText.includes("principal address street") || lowerText.includes("address *")) inferredFieldId = "ar_principal_street"; 
            else if (lowerText.includes("principal address city")) inferredFieldId = "ar_principal_city"; 
            else if (lowerText.includes("principal address state")) inferredFieldId = "ar_principal_state"; 
            else if (lowerText.includes("principal address zip") || lowerText.includes("principal zip")) inferredFieldId = "ar_principal_zip"; 
            else if (lowerText.includes("mailing address selection")) inferredFieldId = "ar_mailing_choice"; 
            else if (lowerText.includes("mailing address street") || lowerText.includes("mailing street")) inferredFieldId = "ar_mailing_street"; 
            else if (lowerText.includes("mailing address city") || lowerText.includes("mailing city")) inferredFieldId = "ar_mailing_city"; 
            else if (lowerText.includes("mailing address state") || lowerText.includes("mailing state")) inferredFieldId = "ar_mailing_state"; 
            else if (lowerText.includes("mailing address zip") || lowerText.includes("mailing zip")) inferredFieldId = "ar_mailing_zip"; 
            else if (lowerText.includes("contact person's full name") || lowerText.includes("contact full name")) inferredFieldId = "ar_contact_name"; 
            else if (lowerText.includes("phone")) inferredFieldId = "ar_contact_phone"; 
            else if (lowerText.includes("email")) inferredFieldId = "ar_contact_email"; 
            else if (lowerText.includes("due date")) inferredFieldId = "ar_state_due_date"; 
            else if (lowerText.includes("state annual report has been filed")) inferredFieldId = "ar_state_filed_choice"; 
            else if (lowerText.includes("city annual report has been filed")) inferredFieldId = "ar_city_filed_choice"; 
            else if (lowerText.includes("federal employer identification")) inferredFieldId = "ar_federal_ein"; 
            else if (lowerText.includes("federal taxes have been filed")) inferredFieldId = "ar_fed_filed_choice"; 
            else if (lowerText.includes("why the state annual filing")) inferredFieldId = "ar_state_reason"; 
            else if (lowerText.includes("why the city annual filing")) inferredFieldId = "ar_city_reason"; 
            else if (lowerText.includes("why the federal tax filing")) inferredFieldId = "ar_fed_reason"; 
            else if (lowerText.includes("other peripheral filings")) inferredFieldId = "ar_other_filings_list"; 
            else if (lowerText.includes("licenses or permits that need to be renewed")) inferredFieldId = "ar_pending_renewals_list"; 
            else if (lowerText.includes("peripheral paperwork filing question")) inferredFieldId = "ar_other_filed_choice"; 
            else if (lowerText.includes("compliance verification check question")) inferredFieldId = "ar_compliance_verified"; 
        ...
        errorsToProcess.forEach(errorItem => { 
            const errMsg = errorItem.message; 
            ...
            if (!fieldInput) { 
                const allVisibleInputs = document.querySelectorAll("#step-2-onboarding-fields-canvas input[required], #step-2-onboarding-fields-canvas select[required], #step-2-onboarding-fields-canvas textarea[required]"); 
    ...
    // ANTI-FLICKER SEGMENT ISOLATION: 
    ...
    // 3. 🟢 FIXED DECOUPLED MATRIX INITIALIZATION: Straight code execution track
...
function cacheAndRestoreWizardFormStatesVanilla(isExecutionInitialLoad) { 
    ...
    // [Function track kept open here intentionally for the restoration loop code underneath it]
...
} // Securely closed the master cacheAndRestoreWizardFormStatesVanilla functional shell scope here
...
} // This completely closes the multi-part functional body of cacheAndRestoreWizardFormStatesVanilla safely
...
if (!window.hasWizardRealTimeStateListenersBound) { 
    document.addEventListener("DOMContentLoaded", () => { 
        ...
        }, 250); // FIX: Extended to 250ms to allow file upload styling and side-by-side elements to mount perfectly first
    }); 
...
function runUnifiedPlatformLifecycleBoot() { 
    ...
    }, 60); // FIX: Extended to 60ms to let elements align side-by-side completely without overlapping text columns
...
function toggleCorporationSharesWorkflow(selectedValue) { 
    ...
    // FIX: Maintain CSS Grid outer system while allowing inner components to layout side by side cleanly
    if (selectedValue === "custom") { 
        ...
        // FIX: Clear hidden field values and remove any hanging validation borders so they don't block advancement gates
...
function toggleCorporationEinReasonField(selectedValue) { 
    ...
    // FIX: Use flex column structure over explicit grid property blocks to maintain field track layouts
...
function handleMembershipDropdownChange(selectElement) { 
    ...
    if (isSingleMember) { 
        // FIX: Removed the stray backslash symbol next to the closing backtick to stop string token corruption errors
        singleMemberBox.innerHTML = ` 
    ...
    if (mainFormSlotNode) { 
        ...
        window.dynamicFormFileObserver = new MutationObserver(() => { 
            ...
            try { 
                ...
                // FIX: Changed from 0ms to 100ms. Gives dynamic script changes time to settle without triggering infinite looping redraws.
                setTimeout(() => { 
                    window.isMutationProcessingActive = false; 
                }, 100); 
            } 
...
function attachStepTwoNavigationTriggers() { 
    ...
    if (continueBtnStep2) { 
        ...
        continueBtnStep2.onclick = function(event) { 
            ...
            // FIX: Context-aware gate check block prevents premature step transitions if validation fails 
            if (typeof window.processStepTwoFunnelAdvancementGate === "function") { 
                // Let the process gate run. It will manage the forward shift itself if valid.
                window.processStepTwoFunnelAdvancementGate(currentEvent); 
    ...
    const triggerSystemPlatformBoot = () => { 
        const verifyAndArmComponents = () => { 
            ...
            if (hasCanvasLoaded) { 
                ...
                // FIX: Changed from requestAnimationFrame to a 100ms timeout loop. 
                // This stops runaway memory loops that duplicate layouts and freeze column track widths.
...
function initializeStep2AssetRouter() { 
    // FIX: Simplified targets to focus exclusively on clean canvas tracks, eliminating layout overlaps
...
async function saveActiveServiceFormStates(fieldsRoot) { 
    ...
    if (currentActiveStepNum === 2) { 
        try { 
            ...
            formFields.forEach(field => { 
                // FIX: Natively skip files since browser security blockades programmatic data reads on file paths
 ...
 <i class="fa-solid fa-spinner fa-spin" style="margin-right: 8px; color: var(--primary, #10b981);"></i> <span>Assembling specialized compliance filing interfaces...</span> </div> 
 </div> `; 
 ...
 <strong>Error Loading System Components:</strong> Could not load file parameters cleanly. 
        ...
        // FIX: Replaced delayed timeouts with direct code routing execution paths to preserve thread safety
        if (typeof window.executeDynamicStepStateHydrationFallback === "function") { 
            window.executeDynamicStepStateHydrationFallback(); 
        } 
    } 

        // ... Continued from the loader pipeline promise block
...
async function executeStepTwoDynamicFormInjection(keysBeforeScriptLoads, rawUrlSlug) { 
    ...
    try { 
        ...
        // [Thread continues fluidly onto Part 2 layout rendering track below]
        ...
        if (!formInjectionWrapper) { 
            formInjectionWrapper = document.createElement("div"); 
            formInjectionWrapper.className = "isolated-form-payload-container"; 
            // FIX: Changed display to 'contents' so the container doesn't trap inner fields or break the parent 2-column grid tracks
        ...
        verifiedTemplates.forEach((item) => { 
            ...
            // FIX: Enforced 'display: contents' so nested sub-inputs can directly participate in the parent side-by-side grid tracks
...
} // Safe, absolute function termination closed here cleanly.
...
function executeStepTwoIsolatedRenderPass() { 
    ...
    formInjectionWrapper.className = "isolated-form-payload-container"; 
    // FIX: Changed display to 'contents' so the container doesn't trap inner fields or break the parent 2-column side-by-side grid tracks
    formInjectionWrapper.style.cssText = "grid-column: 1 / -1 !important; width: 100% !important; max-width: 100% !important; display: contents !important; clear: both !important;"; 
...
function executeStepTwoFormHydrationPipeline() { 
    ...
    // 🛡 RECOVERY LIFECYCLE GATE: Isolated element extraction 
    const wrapper = document.getElementById('formInjectionWrapper') || document.getElementById('step-2-onboarding-fields-canvas') || document.getElementById('step-2-injection-placeholder'); 
    if (!wrapper) { 
        console.warn("[Step 2 Engine Recovery] DOM container missing from workspace viewport. Skipping background layout loop."); 
        return; 
    } 
    ...
    // [Thread continues fluidly onto Part 2 layer formatting below]
    ...
    step2IsolatedTemplates.forEach((item) => { 
        ...
        if (!existingRow) { 
            ...
            // FIX: Enforced 'display: contents' so nested sub-inputs can participate directly in the parent side-by-side grid tracks
            existingRow.style.cssText = "grid-column: 1 / -1 !important; display: contents !important; width: 100% !important; max-width: 100% !important; clear: both !important; box-sizing: border-box;"; 
        ...
        // FIX: Enforced 'display: contents' on inner carrier wrapper to keep formatting column tracks from collapsing
        internalCarrierNode.style.cssText = "grid-column: 1 / -1 !important; display: contents !important; width: 100%; clear: both;"; 
    ...
    // 🛡 ANTI-FLICKER RENDERING REBOOT: Avoid destructive re-appending loops that detach live Step 3 elements 
...
window.toggleFederalTaxInventoryCostVisibility = function(targetSelectorOrEvent, programmaticFallbackNode, runSynchronously = true) { 
    ...
    // 4. Final attribute extraction attempt if not already captured if not already captured 
    ...
    const executeMutationLogicCore = () => { 
        ...
        if (targetContainer) { 
            ...
            // FIX: Re-routed the data save pass call context to your standard operational cache wrapper function
            if (typeof window.saveWizardFormStatesVanilla === "function") { 
                window.saveWizardFormStatesVanilla(); 
            } 
        } else { 
...
// ============================================================================ // 
// 🛠 DYNAMIC MARKUP TEMPLATE FIELDS HYDRATION SYSTEM (ANTI-LOOP ENGINE)         // 
// ============================================================================ // 
function hydrateInjectedFormFields(formInjectionWrapper) { 
    ...
    try { 
                ...
                if (savedCacheStringValue !== null) { 
                    ...
                    if (elementItem.tagName === "SELECT") { 
                        ...
                        if (hasMatchingOption || elementItem.options.length > 1) { 
                            ...
                            // FIX 2: Replaced 0ms setTimeout with straight code execution pass to keep layout side-by-side tracks stable
                    ...
                    // FIX 3: Safely route to visibility controllers directly instead of firing heavy bubbling event cascades 
                    if (elementItem.isConnected) { 
                        try { 
        ...
        // [Thread continues fluidly onto Part 2 radio/checkbox layer processing below]
        ...
        if (activeStepContext === 3 || containsStepThreeFields) { 
            if (typeof window.updateDynamicPricingMatrixVanilla === "function") { 
                ...
                // Run synchronously to ensure layout forms don't collide or experience race loops
...
// ============================================================================ // 
// 🛠 SERVICE FORM DYNAMIC FIELD PATCHER & DATA INTERCEPTOR (STABILIZED)        // 
// ============================================================================ // 
function serializeAndPatchActiveServiceFields() { 
    console.log("[Data Matrix] Dynamically serializing current service form fields..."); 
    
    // FIX: Prioritize your strict fields canvas layout to avoid shifting parent layout panels 
    const formContainer = document.getElementById("step-2-onboarding-fields-canvas"); 
    
    if (formContainer) { 
        // FIX: Temporarily pause the global mutation tracking engine to prevent an infinite mutation feedback cycle 
 ...
 <input type="text" id="global_company_owner" name="global_company_owner" required class="wizard-input-field" style="font-size: 0.95rem !important; height: 44px !important; padding: 10px 14px !important; width: 100% !important; border: 1px solid #cbd5e1; border-radius: 6px; box-sizing: border-box !important;"> `; 
            ...
            // FIX: Re-engage the MutationObserver strictly back to your isolated fields canvas root 
    ...
    activeFormFields.forEach(field => { 
        ...
        if (field.type === 'checkbox') { 
            localStorage.setItem(`wizard_field_${checkboxIdentifier || fieldIdentifier}`, field.checked ? "true" : "false"); 
        } else { 
...
window.executeDynamicAddonCompilation = function(targetFieldRef, explicitContextOverride) { 
 ...
 <span>+ ${addon.name}</span> <span style="font-family: monospace; font-weight: 700; color: #10b981;">$${fixedItemPrice.toFixed(2)}</span> 
 </div>`; 
...
async function runStepTwoLayoutInitialization() { 
    ...
    // FIX: Clear layout polling request frames and use straight code handling to prevent recursive background loops
    ...
    try { 
        ...
        if (typeof finalFormHydratorEngine === "function") { 
            ...
            // 2. FIXED DEBOUNCED CALCULATION COUPLING: Extended delay block path
            // Provide an 80ms macrotask window to allow dynamic DOM fields to establish 
            // their layout structures and saved inputs completely before performing summary price sweeps.
            setTimeout(() => { 
...
function enforceStrictDomDeduplication(wrapperElement) { 
    ...
    sectionHeaders.forEach(header => { 
        ...
        if (encounteredTitles.has(titleText)) { 
            // 🛡 SECURITY SEPARATION CHECK: Verify this is a trailing clone, not the primary segment block 
...
function bootstrapStepTwoLifecycleEngine() { 
    ...
    if (isPluginDataReady) { 
        ...
        window.formRegistry = new Proxy(baseTargetRegistry, { 
            ...
            // [Thread moves fluidly onto Part 2 proxy setter and observer tracking loops below]
        ...
        // FIX: Target only strict template canvas containers to prevent row tracking track overlaps
...
(function() { 
    ...
    function executeDynamicAddonCompilation() { 
        ...
        // FIX: Scrape active inputs safely within step-2 template canvas targets exclusively to avoid cross-talk
        const step2Canvas = document.getElementById("step-2-onboarding-fields-canvas") || 
                            document.getElementById("step-2-injection-placeholder"); 
