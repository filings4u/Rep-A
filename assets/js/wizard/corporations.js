/**
 * SYSTEM COMPLIANCE SERVICE: CORPORATE FORMATION ENGINE
...
function initCorporateFormationServices() {
  ...
  window.formRegistry['corporations-part1-validation'] = {
    ...
    validate: function() {
      ...
      // Corporate name validation structure rule check
      ...
      if (wrapper && (wrapper.style.display === "grid" || wrapper.style.display === "block" || (raField && raField.value === "custom"))) {
        ...
        // Custom agent zip formatting check
      ...
      document.querySelectorAll("input[id^='shareholder_first_name_']").forEach(inputEl => {
        if (isVis(inputEl)) {
          ...
          (!inputEl.value.trim()) ? markInvalid(inputEl, errNode, `Shareholder #${idx} first name is required.`) : markValid(inputEl, errNode);
        }
      });
      ...
      document.querySelectorAll("input[id^='shareholder_last_name_']").forEach(inputEl => {
        if (isVis(inputEl)) {
          ...
          (!inputEl.value.trim()) ? markInvalid(inputEl, errNode, `Shareholder #${idx} last name is required.`) : markValid(inputEl, errNode);
        }
      });
...
window.formRegistry['corporations-part1-layout'] = function(stateDropdownOptionsHtml = "") {
  ...
  <div style="grid-column: span 2; background: rgba(10, 31, 68, 0.03); border-left: 4px solid var(--navy, #0a1f44); padding: 14px; border-radius: 0 8px 8px 0; font-size: 0.8rem; line-height: 1.4; color: var(--slate, #64748b); box-sizing: border-box; margin-bottom: 12px;">
    <strong style="color: var(--navy, #0a1f44); display: block; margin-bottom: 4px;"><i class="fa-solid fa-circle-info"></i> What is a Corporation Formation?</strong>
    Incorporating sets up a distinct legal personhood asset envelope. This structure separates corporate assets from personal assets, shields equity positions, and establishes a rigid authorized share tracking matrix natively.
  </div>

  <div style="grid-column: span 2; border-bottom: 1px solid var(--border, #e2e8f0); padding-bottom: 8px; margin-top: 12px;">
    <h3 style="color: var(--navy, #0a1f44); font-size: 1.1rem; font-weight: 800; margin: 0;">1. Corporate Business Information</h3>
  </div>
  ...
  <div style="grid-column: span 2; background: rgba(10, 31, 68, 0.03); border-left: 4px solid var(--navy, #0a1f44); padding: 14px; border-radius: 0 8px 8px 0; font-size: 0.8rem; line-height: 1.4; color: var(--slate, #64748b); box-sizing: border-box; margin-bottom: 4px;">
    ...
    States require corporations to designate a local physical location receiver. This person or service must stay open during regular business hours to accept legal notices, Service of Process (SOP), and state updates.
  </div>

   <!-- FIELD 3: REGISTERED AGENT PROVISION DROPDOWN (FIXED TEXT VISIBILITY) -->
...
window.formRegistry['corporations-shareholder-layout'] = function() {
  ...
  <div style="grid-column: span 2; background: rgba(10, 31, 68, 0.03); border-left: 4px solid var(--navy, #0a1f44); padding: 14px; border-radius: 0 8px 8px 0; font-size: 0.8rem; line-height: 1.4; color: var(--slate, #64748b); box-sizing: border-box; margin-bottom: 12px;">
    <strong style="color: var(--navy, #0a1f44); display: block; margin-bottom: 4px;"><i class="fa-solid fa-circle-info"></i> What is a Shareholder?</strong> 
    A shareholder is an individual or entity that owns stock units inside a corporation. State statutes mandate cataloging initial owners to accurately allocate initial corporate asset boundaries on public records.
  </div>
      ...
      <div class="member-record-card" id="shareholder_card_1" style="background: #ffffff; border: 1px solid var(--border, #e2e8f0); padding: 16px; border-radius: 8px; width: 100%; box-sizing: border-box;"> 
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; margin-top: 12px;"> 
          ...
          <div class="wizard-input-group" style="grid-column: span 1; margin: 0; display: flex; flex-direction: column; gap: 6px;"> 
            ...
            <input type="text" id="shareholder_unit_1" placeholder="e.g. Suite 400" class="wizard-input-field" style="width: 100%; box-sizing: border-box; padding: 12px; border: 1px solid #cbd5e1; border-radius: 6px;"> 
          </div> 
          ...
          <div class="wizard-input-group" style="grid-column: span 1; display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin: 0;"> 
            <div style="display: flex; flex-direction: column; gap: 6px;"> 
              ...
              <select id="shareholder_state_1" required class="wizard-input-field" style="width: 100%; box-sizing: border-box; height: 38px; font-weight: 600; background-color: #ffffff; padding: 0 12px; border: 1px solid #cbd5e1; border-radius: 6px;"> 
                ${typeof window.buildGlobalUsaStateDropdownOptionsHtml === 'function' ? window.buildGlobalUsaStateDropdownOptionsHtml("") : '<option value="WY">Wyoming</option>'} 
              </select> 
  ...
  <div style="grid-column: span 2; margin-top: 12px; margin-bottom: 20px;"> 
    <button type="button" id="btn_add_shareholder" class="wizard-button-secondary" style="font-weight:700; cursor: pointer; padding: 10px 20px; border: 1px solid #cbd5e1; background: #ffffff; border-radius: 6px; display: inline-flex; align-items: center; gap: 6px;">
      <i class="fa-solid fa-plus" style="color: var(--primary, #10b981);"></i> + Add Additional Shareholder
    </button> 
  </div> 
  `;
};
...
window.formRegistry['corporations-part2-layout'] = function(stateDropdownOptionsHtml = "") {
  ...
  <div style="grid-column: span 2; background: rgba(10, 31, 68, 0.03); border-left: 4px solid var(--navy, #0a1f44); padding: 14px; border-radius: 0 8px 8px 0; font-size: 0.8rem; line-height: 1.4; color: var(--slate, #64748b); box-sizing: border-box; margin-bottom: 12px;">
    <strong style="color: var(--navy, #0a1f44); display: block; margin-bottom: 4px;"><i class="fa-solid fa-circle-info"></i> What is Authorized Stock and Par Value?</strong>
    Authorized shares represent the max units a corporation can legally issue. Par value is an arbitrary baseline dollar value assigned to each stock piece for internal structural equity calculations (standard baseline is 0.0001).
  </div>
  ...
  <div class="wizard-input-group" style="grid-column: span 1; display: flex; flex-direction: column; gap: 6px;">
    ...
    <!-- FIELD 3: S-CORP ELECTION DROPDOWN (FIXED TEXT VISIBILITY) -->
  <div class="wizard-input-group" style="grid-column: span 2; display: flex; flex-direction: column; gap: 6px;">
  ...
  <!-- HIDDEN CONDITIONAL CONTAINER: S-CORP FORM 2553 PREMIUM ADDON HOOK (FIXED TEXT VISIBILITY) -->
...
/**
 * 🔌 CONTROLLER 1: REGISTERED AGENT DISPLAY INTERLOCK
 * Handles visibility and strict layout constraints for the custom agent form workspace.
 */
...
/**
 * 📦 DYNAMIC SHAREHOLDER REGISTRY MANAGER
 * Handles instant node instantiations and appends functional remove click listeners.
 */
setTimeout(() => {
  const appendBtn = document.getElementById("btn_add_shareholder");
  if (!appendBtn) return;
  ...
  appendBtn.addEventListener("click", () => {
    ...
    const stateOptions = typeof window.buildGlobalUsaStateDropdownOptionsHtml === 'function' ? window.buildGlobalUsaStateDropdownOptionsHtml("") : '<option value="WY">Wyoming</option>';
    ...
    // Inject side-by-side data tracks identical to Shareholder Card #1
    cardNode.innerHTML = `
    ...
    if (newZipInput) {
      newZipInput.addEventListener('input', function() { this.value = this.value.replace(/\D/g, ''); });
    }
  });
}, 400);
 ...
 * Safely removes a shareholder card container and re-indexes the layout stack.
...
window.removeShareholderCardNode = function(cardIdIndex) {
  ...
  remainingCards.forEach((card, loopIndex) => {
    // Retain Card #1's hardcoded default state layout intact
    if (card.id === "shareholder_card_1") return;
    ...
    if (trashButton) {
      ...
      // 🌟 DYNAMIC DUAL PASS NODE SCANNER: Loops over all active shareholder card entries
      document.querySelectorAll("input[id^='shareholder_first_name_']").forEach(inputEl => {
        if (isVis(inputEl)) {
          const idx = inputEl.id.replace("shareholder_first_name_", "");
      ...
      document.querySelectorAll("input[id^='shareholder_street_']").forEach(inputEl => {
        if (isVis(inputEl)) {
          ...
          (!inputEl.value.trim()) ? markInvalid(inputEl, errNode, `Shareholder #${idx} street address is required.`) : markValid(inputEl, errNode);
        }
      });
      ...
      document.querySelectorAll("input[id^='shareholder_city_']").forEach(inputEl => {
        if (isVis(inputEl)) {
          ...
          (!inputEl.value.trim()) ? markInvalid(inputEl, errNode, `Shareholder #${idx} city is required.`) : markValid(inputEl, errNode);
        }
      });
      ...
      document.querySelectorAll("select[id^='shareholder_state_']").forEach(selectEl => {
        if (isVis(selectEl)) {
          ...
          (!selectEl.value.trim()) ? markInvalid(selectEl, errNode, `Please select a state for Shareholder #${idx}.`) : markValid(selectEl, errNode);
        }
      });
      ...
      document.querySelectorAll("input[id^='shareholder_zip_']").forEach(inputEl => {
        if (isVis(inputEl)) {
          ...
          if (!inputEl.value.trim()) {
            markInvalid(inputEl, errNode, `Shareholder #${idx} zip code is required.`);
          } else if (!/^\d{5}$/.test(inputEl.value.trim())) {
            markInvalid(inputEl, errNode, `Shareholder #${idx} zip code must be exactly 5 digits.`);
          } else {
