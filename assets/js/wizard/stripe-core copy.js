...
async function processCheckoutHandshake() { 
    ...
    // 1. DYNAMIC API REQUEST ENVELOPE (Includes runtime operational control keys needed by the Edge Function)
    var edgeFunctionPayload = { 
        ...
        total_paid_amount: parseFloat(targetAmount || 0).toFixed(2), 
        poa_signature: poaSignature.trim(), 
        poa_execution_stamp: new Date().toISOString(), 
        company_name: companyName.trim(),
        
        // ðŸŽ¯ THE OPERATIONAL COUPLING INJECTION:
        // These fields are passed exclusively to the API call so the backend validation does not abort.
        // They are NOT saved to the public.orders table directly from your frontend scripts.
        action_intent: "initialize_payment_intent",
        status: "initiated"
    }; 

    console.log("ðŸ“¡ [Supabase Gateway] Dispatching secure operational payload to live Edge Function...");
    try { 
        var responseStream = await fetch("https://lrbimrlbskjweynxlgas.supabase.co/functions/v1/stripe-webhook", { 
            method: "POST", 
            headers: { "Content-Type": "application/json" }, 
            body: JSON.stringify(edgeFunctionPayload) // Transmits payload securely over the network
        }); 
        ...
        // Update the clean table-mapped master payload model for step-7 references
        ...
        console.log("âœ… [Handshake Envelope] Pristine Checkout Session token cached cleanly."); 
        ...
        console.error("ðŸš¨ [Handshake Fault]: ", handshakeFault.message); 
        const errorBanner = document.getElementById("step6-error-banner-target"); 
        if (errorBanner) { 
            errorBanner.style.display = "block"; 
            errorBanner.innerHTML = `ðŸš¨ <strong>Handshake Link Error:</strong> ${handshakeFault.message}`; 
        } 
...
function applyStep6DisplayReflowAdjustments(panelNode, targetStepInt) { 
    ...
    // FIX: Force layout container back to standard display block model.
    // This stops parent collision freezes and allows navigation buttons to occupy active screen height.
    ...
    // Defer layout calculation to an animation frame to avoid thread thrashing 
