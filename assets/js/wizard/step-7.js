...
(function() { 
    ...
    window.formRegistry['account-creation-layout'] = function(stateOptionsHtml, prefilledEmail) { 
        ...
        '<select id="state" style="width: 100%; padding: 12px; border: 1px solid #cbd5e1; border-radius: 6px; height: 45px; background: white; box-sizing: border-box; text-align: left;">' + 
        states + 
        '</select>' + 
    ...
    window.bindFormSubmissionEvents = function() { 
        ...
        formElement.addEventListener("submit", function(event) { 
            ...
            .then(function(resolvedUserId) { 
                ...
                // 🟢 CRITICAL FIX: Explicitly return the Supabase promise so the chain waits for completion 
            ...
            .then(function(profileResponse) { 
                // Check if the previous database write threw an error 
                if (profileResponse && profileResponse.error) { 
                    console.warn("Profile sync warning:", profileResponse.error.message); 
                } 
                if (targetTrackingNumber) { 
                    ...
                    // 🟢 CRITICAL FIX: Return the update promise 
                    return window.supabase 
                        .from("orders") 
                        .update({ 
            ...
            .then(function(ordersResponse) { 
                if (ordersResponse && ordersResponse.error) { 
                    console.warn("Orders sync warning:", ordersResponse.error.message); 
                } 
                console.log("[Edge Function] Invoking stripe-webhook transactional email manager worker..."); 
                // 🟢 CRITICAL FIX: Return the invoke promise so its failure routes straight to the final .catch block 
            ...
            .catch(function(runtimeError) { 
                // 🟢 EXCEPTION SHIELD: If CORS blocks the edge function, catch it here and force step transition anyway 
                console.error("Critical submission processing bypass engaged (Pipeline Error):", runtimeError); 
                localStorage.setItem("stripe_checkout_registered_userid", userEmail); 
                if (window.f4uMasterInstanceApp) { 
                    window.f4uMasterInstanceApp.currentStep = 8; 
                } 
