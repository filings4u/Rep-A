// ============================================================================ //
// 🎨 LAYER 4: PREMIUM LANDING PAGE ANIMATION CARD ENGINE STYLES                //
// ============================================================================ //
...
// ============================================================================ //
// 🎛️ LAYER 5: CENTRAL PRICING RESOLUTION BRIDGE                                //
// ============================================================================ //
function resolveCentralPricingObject(slug) { 
    // 🟢 DATA-DRIVEN: Targets your explicit master core database object directly 
    const sourceMatrix = window.CENTRAL_SERVICE_PLAN_DB || {}; 
    const safeSlug = slug ? slug.toLowerCase().trim() : ""; 
    
    // Look up via alternative token dash/underscore casing keys
...
// ============================================================================ //
// 🛠️ LAYER 6: MASTER AUTOMATED PRICING GRAPHICS INTERFACE RENDERER             //
// ============================================================================ //
function renderMasterPricingEngine(overrideTargetId, metaDataRecord) { 
    try { 
        ...
        const meta = { 
            slug: slug, 
            title: contextSource.title || contextSource.hero_title || dynamicPrices.name || slug.replace(/-/g, ' ') 
        }; 
...
function executePremiumAnimatedPackagesGrid(zone, meta, dynamicPrices) { 
    ...
    // Extract live url parameters to carry the registration state forward
    ...
    plansConfig.forEach(function(plan) { 
        ...
        var badgeHtml = isComp ? '<span style="position:absolute;top:-12px;left:50%;transform:translateX(-50%);background:#10b981;color:#fff;font-weight:800;font-size:0.72 tram;padding:4px 12px;border-radius:20px;z-index:10;font-family:sans-serif;letter-spacing:0.5px;">MOST POPULAR</span>' : ''; 
        var btnBg = isComp ? '#10b981' : '#0a1f44'; 
        
        // Dynamic bullets parsing matrix
        ...
        // 🟢 FIXED PARAMETER ROUTING: Appends the selected state context directly onto the button href values
