...
try {
    ...
    if (!readerClientInstance) {
        // Find the base library object safely
        const supabaseLib = window.supabase || (typeof supabase !== 'undefined' ? supabase : null);
        
        if (supabaseLib && typeof supabaseLib.createClient === 'function') {
            // Instantiate once and cache it on the window to protect subsequent scripts
    ...
    if (!activeSlug) {
        renderErrorState("Error: No valid article resource specified in URL parameters.");
        return;
    }
  ...
  async function fetchArticleBodyPayload(slugString) {
    const contentStream = document.getElementById('mainArticleContentStream');
    ...
    try {
      ...
      if (error || !posts || posts.length === 0) {
        renderErrorState("Article Not Found. The resource may have been deleted or archived by administrators.");
        return;
      }

      const activePost = posts[0];
      const formattedDate = new Date(activePost.created_at).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      });

      const parsedParagraphs = activePost.content 
        ? activePost.content.split('\n').map(p => p.trim() ? `<p>${p}</p>` : '').join('') 
        : `<p>${activePost.summary || 'No summary overview text provided.'}</p>`;

      const definitiveImage = activePost.image_url || 'https://unsplash.com';

      if (contentStream) {
        contentStream.innerHTML = `
          <a href="blog.html" class="btn-back-link">← Return to Insights Feed</a>
          <img src="${definitiveImage}" alt="${activePost.title}" class="article-hero-image" onerror="this.onerror=null; this.src='images/logo-white.png'; this.style.padding='40px'; this.style.objectFit='contain'; this.style.background='#0a1f44'; this.style.maxHeight='300px';">
          <div class="article-header-meta">
            <span class="category-tag" style="background: ${activePost.category === 'education' ? '#0a1f44' : '#10b981'} !important;">
