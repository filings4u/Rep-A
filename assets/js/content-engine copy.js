...
window.PLATFORM_STATIC_PAGES_DATA = window.PLATFORM_STATIC_PAGES_DATA || {
  ...
  "index_metrics": {
    ...
    cards: [
      { icon: "🏢", value: "142K+", title: "Corporate Entities Formed", desc: "Authorized Articles of Organization across all 50 State Secretary registries." },
      { icon: "🚛", value: "38,410", title: "Active Transits Monitored", desc: "USDOT & MC operating authorities actively synchronized with FMCSA core data links." },
      { icon: "⚡", value: "1.8s", title: "Average API Pipeline Turn", desc: "Secure, real-time rest requests to launch bank check intents and background pre-saves.", green_text: true },
      { icon: "🔒", value: "99.98%", title: "Filing Accuracy Quotient", desc: "Sophisticated layout rules eliminate common syntax rejection errors from state systems." }
    ]
  },
  "index_alternating": {
  ...
  "index_expertise": {
    ...
    events: [
      { title: "Dedicated Account Liaison", desc: "Direct human routing for complex state business applications", status: "INCLUDED" },
      { title: "Pre-Submission Document Audits", desc: "Our specialists cross-verify address and entity name spelling rules", status: "VERIFIED" },
      { title: "Automated Franchise Tax Alerts", desc: "Proactive neighborhood deadline notifications via phone or email", status: "LIVE" }
    ]
  },
  "index_startup": {
...
// 🏢 LLC SERVICE PAGE METRICS OBJECT BLOCK DEFINITION
  "limited-liability-company_metrics": {
    section_title: "LLC Formation Performance Link",
    status_badge: "STATE CHANNELS ONLINE",
    cards: [
      { icon: "⚡", value: "24 Hrs", title: "Average Filing Spin", desc: "Expedited processing returns official company articles by the next business morning." },
      { icon: "🛡️", value: "100%", title: "Asset Protection Shield", desc: "Corporate structures configured strictly to shield your personal wealth profile." },
      { icon: "📄", value: "EIN Inbound", title: "Automated IRS Link", desc: "Direct federal network processing quickly generates your commercial tax identity." },
      { icon: "👥", value: "Active", title: "Registered Agent Sync", desc: "Includes ongoing address routing logs to guarantee perfect administrative health." }
    ]
  },
...
function renderDynamicHeroSection(targetElementId) {
  ...
  if (!detectedPageKey || detectedPageKey === "index copy" || detectedPageKey === "home" || urlPathname === "/") {
  ...
  // 🎯 SPACING DIALS FOR INTERNAL SUBPAGES ONLY (Homepage remains untouched)
  const innerPageDesktopPaddingTop = "10px"; // Decrease this to pull desktop pages up further
  const innerPageMobilePaddingTop = "10px";  // Decrease this to pull mobile pages up further
  ...
  node.innerHTML = `
    ...
    <style>
      ...
      /* 🌐 SPECIFIC TARGETED INNER PAGE ADJUSTMENTS (Does not alter the index page) */
      ${!isIndexPage ? `
      .hero-section-wrapper {
        padding-top: ${innerPageDesktopPaddingTop} !important;
      }
      ` : ''}
      ...
      @media (max-width: 991px) {
        ...
        /* 🌐 SPECIFIC TARGETED MOBILE INNER PAGE ADJUSTMENTS */
        ${!isIndexPage ? `
        .hero-section-wrapper {
          padding-top: ${innerPageMobilePaddingTop} !important;
        }
        ` : ''}
        ...
        /* 🎯 THE MOBILE WORD-WRAPPING FIX: Injects horizontal breathing space between words and letters across mobile viewports to completely eliminate abrupt word-breaking */
        .responsive-hero-grid h1, .responsive-hero-grid p, .responsive-hero-grid span, .responsive-hero-grid a {
    ...
    <main class="hero-section-wrapper">
      <div class="site-width-alignment-guard">
        <div class="responsive-hero-grid">
          ...
          <!-- Adaptive Preview Image Block Container -->
...
/**
 * Injects a highly adaptive page-specific data metrics block programmatically based on the active URL path.
 * Bridges live database records with local data catalog sheets seamlessly on state load.
 */
function renderDynamicMetricsSection(targetElementId, dbRowData = null) {
  const node = document.getElementById(targetElementId);
  if (!node) return;

  // 1. Resolve active URL path key tokens
  const urlPathname = window.location.pathname;
  let detectedPageKey = urlPathname.split("/").pop().replace(".html", "").trim().toLowerCase();
  ...
  const staticCopy = (window.PLATFORM_STATIC_PAGES_DATA && window.PLATFORM_STATIC_PAGES_DATA[metricsDataKey]) 
    ? window.PLATFORM_STATIC_PAGES_DATA[metricsDataKey] 
    : null;

  // 2. Choose database data records first, fallback to static configurations safely
  let activeSectionTitle = "Filing Operations Framework";
  let activeStatusBadge = "SYSTEM ACTIVE: PIPELINE REST SECURE";
  let activeCardsArray = [];

  if (dbRowData && (dbRowData.metrics_cards || dbRowData.cards)) {
    activeSectionTitle = dbRowData.metrics_title || dbRowData.section_title || "Filing Performance Link";
    activeStatusBadge = dbRowData.metrics_status || dbRowData.status_badge || "DATABASE CHANNELS ONLINE";
    activeCardsArray = dbRowData.metrics_cards || dbRowData.cards;
  } else if (staticCopy) {
    activeSectionTitle = staticCopy.section_title;
    activeStatusBadge = staticCopy.status_badge;
    activeCardsArray = staticCopy.cards;
  } else {
    // Return early if no unique dataset matches the current URL parameters
    return;
  }

  const sanitize = (str) => String(str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

  // 3. Loop through active cards array parameters programmatically
  let cardsHtmlPayload = "";
  if (Array.isArray(activeCardsArray)) {
    activeCardsArray.forEach(card => {
      ...
      cardsHtmlPayload += `
        <div class="metric-card-block" style="background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 30px 24px; box-sizing: border-box; display: flex; flex-direction: column; gap: 8px; transition: border-color 0.3s; width: 100%;" onmouseover="this.style.borderColor='#10b981'" onmouseout="this.style.borderColor='rgba(255,255,255,0.08)'">
          ...
          <div style="font-size: 2.4rem; font-weight: 900; color: ${numericColorValue}; font-family: monospace; line-height: 1;">${sanitize(card.value)}</div>
          <div style="font-size: 0.95rem; font-weight: 800; color: #cbd5e1; margin-top: 4px;">${sanitize(card.title)}</div>
          <p style="margin: 0; font-size: 0.8rem; color: #94a3b8; line-height: 1.5; font-weight: 500;">${sanitize(card.desc)}</p>
        </div>
      `;
    });
  }

  // 4. Inject structural component markup layout rows into target element
  node.innerHTML = `
    <section class="enterprise-metrics-section" style="padding: 80px 0 !important; background: #0a1f44; color: #f4f7fa; width: 100% !important; max-width: 100% !important; box-sizing: border-box; overflow: hidden; position: relative; margin: 0 !important;">
      <style>
        ...
        @media (max-width: 1199px) {
          .metrics-dashboard-grid { grid-template-columns: repeat(2, 1fr) !important; }
        }
        @media (max-width: 767px) {
          .enterprise-metrics-section { padding: 60px 0 !important; }
          .metrics-header-flex-wrapper { flex-direction: column !important; align-items: flex-start !important; gap: 16px !important; }
          .metrics-header-flex-wrapper .text-right-aligned { text-align: left !important; }
          .metrics-dashboard-grid { grid-template-columns: 1fr !important; gap: 20px !important; }
        }
      </style>
      ...
      <div class="site-width-alignment-guard" style="width: 100% !important; max-width: 1450px !important; margin: 0 auto !important; padding: 0 40px !important; box-sizing: border-box !important; position: relative; z-index: 10;">
        ...
        <div class="metrics-header-flex-wrapper" style="display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 1px solid rgba(244,247,250,0.1); padding-bottom: 24px; margin-bottom: 40px; flex-wrap: wrap; gap: 24px; width: 100%; box-sizing: border-box;">
          <div style="text-align: left; max-width: 600px;">
            <h2 style="margin: 0; font-size: 2.2rem; font-weight: 800; color: #ffffff; letter-spacing: -0.5px; line-height: 1.2;">${sanitize(activeSectionTitle)}</h2>
          </div>
          <div class="text-right-aligned" style="text-align: right;">
            <div style="display: flex; align-items: center; gap: 8px; font-size: 0.8rem; font-weight: 700; color: #10b981; font-family: monospace; background: rgba(16,185,129,0.1); padding: 8px 16px; border-radius: 30px; border: 1px solid rgba(16,185,129,0.2); width: fit-content;">
              <span style="width: 8px; height: 8px; background: #10b981; border-radius: 50%; display: inline-block;"></span>
              ${sanitize(activeStatusBadge)}
            </div>
          </div>
        </div>
...
function renderDynamicExpertiseSection(targetElementId) {
  ...
  if (!window.PLATFORM_STATIC_PAGES_DATA || !window.PLATFORM_STATIC_PAGES_DATA[expertiseDataKey]) return;
  ...
  let logsHtmlPayload = "";
  if (Array.isArray(expCopy.events)) {
    expCopy.events.forEach(ev => {
      logsHtmlPayload += `
        <div class="border-glowing-card" style="display: flex; justify-content: space-between; align-items: center; padding: 20px 24px; background: rgba(16, 185, 129, 0.02); border: 1px solid rgba(16, 185, 129, 0.25); border-radius: 8px; box-shadow: 0 0 15px rgba(16, 185, 129, 0.05); gap: 16px; box-sizing: border-box;">
  ...
  node.innerHTML = `
    <section class="enterprise-metrics-section" style="padding: 80px 0 !important; background: #0a1f44; color: #f4f4f4; width: 100% !important; max-width: 100% !important; box-sizing: border-box; overflow: hidden; position: relative; margin: 0 !important;">
      <style>
        @media (max-width: 767px) {
      ...
      <div class="site-width-alignment-guard" style="width: 100% !important; max-width: 1450px !important; margin: 0 auto !important; padding: 0 40px !important; box-sizing: border-box !important; position: relative; z-index: 10;">
        ...
        <div style="display: flex; flex-direction: column; gap: 24px; box-sizing: border-box; width: 100%;">
          ${logsHtmlPayload}
        </div>
      </div>
    </section>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /**
 * Injects the global website navigation header dynamically into a target element container.
 * Includes built-in engine logic to toggle mobile drop panels smoothly without throwing errors.
 */
function renderDynamicGlobalCorporateNavigation(targetElementId) {
  const node = document.getElementById(targetElementId);
  if (!node) return;

  // 1. Render the HTML navigation structure
  node.innerHTML = `
    <nav>
      <div class="nav-content-wrapper">
        ...
        <div class="nav-links" id="global-nav-links-container">
          
          <!-- FORMATIONS DROPDOWN -->
          <div class="nav-item-dropdown static-dropdown">
            <a href="#">Formations <span style="font-size:0.7rem; margin-left:4px;">▼</span></a>
            <div class="dropdown-content mega-panel-three-col">
              ...
              <div class="mega-column">
                <span class="column-title">Expansion & Recovery</span>
                <a href="foreign-qualification.html">Foreign Entity Certificate</a>
          ...
          <!-- COMPLIANCE DROPDOWN -->
          <div class="nav-item-dropdown static-dropdown">
            <a href="#">Compliance <span style="font-size:0.7rem; margin-left:4px;">▼</span></a>
            <div class="dropdown-content mega-panel-three-col">
              <div class="mega-column">
              ...
              <div class="mega-column">
                <span class="column-title">Global Verification</span>
                <a href="good-standing.html">Certificate of Good Standing</a>
                <a href="apostille-services.html">Apostille Authentication</a>
          ...
          <div class="nav-item-dropdown static-dropdown">
            <a href="#">Tax Filings <span style="font-size:0.7rem; margin-left:4px;">▼</span></a>
            <div class="dropdown-content mega-panel-three-col">
              <div class="mega-column">
              ...
              <div class="mega-column">
                <span class="column-title">Gov & Procurement</span>
                <a href="cage-code.html">CAGE Code</a>
                <a href="duns-number.html">DUNS Number Procurement</a>
                <a href="procurement-registration.html">Procurement Registration</a>
          ...
          <!-- DOT & FLEET DROPDOWN -->
          <div class="nav-item-dropdown static-dropdown">
            <a href="#">DOT & Fleet <span style="font-size:0.7rem; margin-left:4px;">▼</span></a>
            <div class="dropdown-content mega-panel-three-col">
              <div class="mega-column">
                ...
                <a href="scac-code.html">SCAC Code Registration</a>
              </div>
              <div class="mega-column">
          ...
          <a href="https://portal.filings4u.com/portal-login.html" class="btn-client-portal">Secure Portal</a>
        </div>
      </div>
    </nav>
  `;

  // 2. ⚡ EMBEDDED INTERNAL MOBILE MENU CONTROLLER SYSTEM
  // Replaces the rigid onclick attributes with modern Event Listeners for optimal safety
  const triggerBtn = node.querySelector("#mobile-menu-trigger");
  const navContainer = node.querySelector("#global-nav-links-container");

  if (triggerBtn && navContainer) {
    triggerBtn.addEventListener("click", () => {
      const isExpanded = triggerBtn.getAttribute("aria-expanded") === "true";
      
      // Toggle states safely
      triggerBtn.setAttribute("aria-expanded", !isExpanded);
      navContainer.classList.toggle("nav-active-mobile-open");
      
      // Change burger character visual accent representation if desired
      triggerBtn.innerHTML = isExpanded ? "☰" : "✕";
    });
  }
}

// 🔍 AUTOMATED CONTENT ENGINE DIAGNOSTICS
(function runMetricsDiagnosticCheck() {
  console.log("⚙️ Running content-engine path diagnostics...");
  const urlPathname = window.location.pathname;
  let detectedPageKey = urlPathname.split("/").pop().replace(".html", "").trim().toLowerCase();
