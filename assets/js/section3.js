/* Fragment 1 of 3: Pricing Layout Styles Setup */
(function injectSection3VisualDesign() {
  ...
  if (!document.getElementById(styleId)) {
    ...
    s.textContent = `
      ...
      #${targetId} .price-badge { position: absolute; top: -14px; left: 50%; transform: translateX(-50%); background: #10b981; color: #fff; padding: 4px 14px; border-radius: 20px; font-size: 0.75rem; font-weight: 800; text-transform: uppercase; white-space: nowrap; }
      ...
      @media (max-width: 991px) { #${targetId} .pricing-grid { grid-template-columns: 1fr!important; gap: 40px!important; } }
    `;
...
/* Fixed Loop Segment: Drops duplicate script-side badgeHtml generation */
plansConfig.forEach(function(plan) {
  ...
  /* FIXED: Changed this from an inline div layout string to an empty string. 
     The CSS ::before selector now handles the full visual placement safely. */
  var badgeHtml = ''; 
...
/* Fragment 3 of 3: HTML Compiler Layout Array Loop */
