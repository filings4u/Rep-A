// 1. Unified Router Translator Key Logic
...
// 2. Core Layout Section Builders
...
async function renderMasterSystem() {
    ...
    const { data, error } = await supabase.from('services').select('sections').eq('slug', activeKey).single();
